...
class DQNAgent(Node):
    def __init__(self, stage, model_name):
        ...
        ************************************************************"""
        ...
        self.load_episode = 0
        self.model_weights_path = os.path.dirname(os.path.realpath(__file__))
        self.model_weights_path = self.model_weights_path.replace(
            'turtlebot3_dqn/dqn_agent',
            'model/weights/MADDPG')
        ...
        if self.load_model:
            self.model = MADDPG(
                ...
                alpha=1e-4,
                beta=1e-3,
                chkpt_dir=self.model_weights_path
            )
            # self.model.set_weights(torch.load(self.model_path).get_weights())
    ...
    *******************************************************************************"""
    ...
    def get_action(self, state):
        ...
        self.model.choose_action(obs,evaluate=True)
